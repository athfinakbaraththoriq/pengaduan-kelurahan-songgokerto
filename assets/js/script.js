...
// Nomor WhatsApp resmi Kelurahan Songgokerto.
// Simpan hanya angka, tanpa "+" atau spasi.
const nomorWhatsApp = "6285713889484";

/**
 * Menormalkan nomor WhatsApp ke format internasional Indonesia (62xxxxxxxxxx).
 * Bisa menerima:
 *   0857xxxxxxxx
 *   857xxxxxxxx
 *   62857xxxxxxxx
 *   +62857xxxxxxxx
 */
function normalisasiNomorWhatsApp(nomor) {
 ...
 *
 * Gunakan endpoint Click-to-Chat resmi WhatsApp melalui api.whatsapp.com.
 * Tidak memakai window.open karena popup blocker browser dapat membuat
 * tombol terasa gagal. URL dibentuk dengan URL + searchParams agar seluruh
 * isi pesan di-encode dengan benar.
 *
 * Catatan: ini bukan WhatsApp Business API. Website hanya membuka chat
 * dengan pesan yang sudah diisi; pengguna tetap menekan "Kirim" di WhatsApp.
 */
function bukaWhatsApp(nomor, pesan) {
  const phone = normalisasiNomorWhatsApp(nomor);

  if (!/^62\d{9,13}$/.test(phone)) {
    showToast("Nomor WhatsApp tujuan belum valid.", true);
    return false;
  }

  const url = new URL("https://api.whatsapp.com/send");
  url.searchParams.set("phone", phone);
  url.searchParams.set("text", String(pesan || ""));

  // Navigasi langsung lebih sederhana dan tidak terkena popup blocker.
